...
{
	import com.atmospherebbdo.util.displayobject.globalToLocalRotation;
	import com.atmospherebbdo.util.displayobject.localToGlobalRotation;
	...
	public class Particle2DUtils 
	{
		 ...
		 * 
		 * This method moves all the display objects into the emitter's renderer,
		 ...
		 * been added to the display list.
		 * 
		...
		public static function createParticle2DFromDisplayObject( obj:DisplayObject, renderer:DisplayObject = null, factory:ParticleFactory = null ):Particle2D
		{
			...
			if ( obj.parent && renderer )
			{
				...
				particle.x = p.x;
				particle.y = p.y;
				var r:Number = globalToLocalRotation(renderer, localToGlobalRotation(obj, 0));
				particle.rotation = degreesToRadians(r);
			...
			else
			{
				particle.x = obj.x;
				particle.y = obj.y;
				particle.rotation = degreesToRadians(obj.rotation);
			}
			particle.image = obj;
